...
if errorlevel 1 (
    ...
    if errorlevel 1 (
        ...
        echo Usando PyInstaller directamente...
        goto :use_pyinstaller_direct
    )
    
    :: Verificar si se instaló correctamente
    poetry --version >nul 2>&1
    if errorlevel 1 (
        echo ERROR: Poetry no se instaló correctamente
        echo Usando PyInstaller directamente...
        goto :use_pyinstaller_direct
    )
)
...
echo.
echo [2/5] Instalando dependencias con Poetry...
poetry install
...
echo.
echo [3/5] Generando ejecutable con Poetry + Flet...
...
if errorlevel 1 (
    echo ERROR: Fallo en la generación del ejecutable con Poetry + Flet
    echo Intentando con PyInstaller...
    goto :use_pyinstaller_poetry
)

echo ✓ Ejecutable generado exitosamente con Poetry + Flet
...
:use_pyinstaller_poetry
echo.
echo [3/5] Generando ejecutable con Poetry + PyInstaller...
...
echo Compilando con Poetry + PyInstaller...
poetry run pyinstaller "%SPEC_FILE%" --noconfirm --log-level=INFO

if errorlevel 1 (
    echo ERROR: Fallo en la generación del ejecutable con Poetry + PyInstaller
    echo Intentando con PyInstaller directamente...
    goto :use_pyinstaller_direct
)

echo ✓ Ejecutable generado exitosamente con Poetry + PyInstaller
...
:use_pyinstaller_direct
echo.
echo [3/5] Generando ejecutable con PyInstaller directamente...
echo Verificando archivo .spec...
...
:continue_build
echo.
echo [4/5] Verificando archivos de instalación...
...
if exist "%DIST_DIR%\%APP_NAME%.exe" (
    ...
    set EXE_FOUND=true
    set EXE_TYPE=pyinstaller
)

if "%EXE_FOUND%"=="false" (
    echo ERROR: No se encontró ningún ejecutable generado
    pause
    exit /b 1
)

:: Copiar ejecutable según el tipo
if "%EXE_TYPE%"=="flet" (
