...
class StatisticsView(ModuleView):
    def __init__(self, page: ft.Page, controller):
        ...
        self._report_filters_card = None
        self._metrics_cards_section = None
        self._charts_section = None
        
        # Componentes críticos que se crean inmediatamente
    ...
    def _create_ui_components_lazy(self):
        """Crea los componentes de UI solo cuando se necesitan (lazy loading)"""
        if self._ui_components_created:
            return
        ...
        # Tarjetas de Métricas con valores por defecto para carga rápida
        ...
        self.total_annual_income_card = self._create_summary_card(
            ...
            description=f"Ingresos {self.current_year}", color=ft.colors.TEAL_500
        )
        
        # Marcadores de posición para gráficos (lazy loading)
    ...
    def _validate_report_generation(self, e):
        ...
        # Asegurar que los componentes existan antes de usarlos
        if not hasattr(self, 'report_type_dropdown') or not hasattr(self, 'generate_report_button'):
            return
    ...
    def build(self):
        """Construye la interfaz con lazy loading para mejorar el rendimiento"""
        try:
            # Crear componentes UI solo cuando se necesitan
            self._create_ui_components_lazy()
            
            # Configurar event handlers (con verificación de existencia)
            if hasattr(self, 'report_type_dropdown'):
                self.report_type_dropdown.on_change = self._validate_report_generation
            if hasattr(self, 'start_date_picker') and hasattr(self, 'start_date_text'):
                self.start_date_picker.on_change = lambda e: self._update_date_text(self.start_date_text, self.start_date_picker.value)
            if hasattr(self, 'end_date_picker') and hasattr(self, 'end_date_text'):
            ...
            # Usar lazy loading para secciones pesadas
            if not self._report_filters_card:
                self._report_filters_card = self._build_report_filters_card()
            
            if not self._metrics_cards_section:
                self._metrics_cards_section = self._build_metrics_cards_section()
                
            if not self._charts_section:
                self._charts_section = self._build_charts_section()

            statistics_panel = ft.Container(
                content=ft.Column([
                    ft.Text("Métricas Clave", size=22, weight=ft.FontWeight.BOLD, color=ft.colors.BLACK87),
                    self._metrics_cards_section,
    ...
    def _build_report_filters_card(self):
        """Construye la sección de filtros de reportes"""
        return ft.Container(
            content=ft.Column([
    ...
    def _build_metrics_cards_section(self):
        """Construye la sección de tarjetas de métricas"""
